    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled?"bg-[#060608]/92 backdrop-blur-xl border-b border-[#1c1c24]":""}`}>
      <div className="max-w-[1380px] mx-auto px-6 md:px-10 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {links.map(([l,h,blank])=>(
            <a key={h} href={h} {...(blank?{target:"_blank",rel:"noopener noreferrer"}:{})} className="px-4 py-2 text-sm text-[#555] hover:text-white rounded-full hover:bg-white/4 transition-all font-mono">{l}</a>
          ))}
        </nav>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 border-b border-[#1c1c24] bg-[#060608] ${open?"max-h-80":"max-h-0"}`}>
        <nav className="flex flex-col gap-1 px-6 py-4">
          {links.map(([l,h,blank])=>(
            <a key={h} href={h} onClick={()=>setOpen(false)} {...(blank?{target:"_blank",rel:"noopener noreferrer"}:{})} className="py-3 text-[#666] hover:text-white text-sm border-b border-[#111] transition-colors font-mono">{l}</a>
          ))}
          <a href="https://docs.google.com/document/d/15r1EynvQedLbK33Ajb6rcDsY7rWMPZUH" target="_blank" rel="noopener noreferrer" onClick={()=>setOpen(false)} className="mt-3 btn-shimmer py-3 rounded-full text-center text-sm font-mono">$ View resume</a>
        </nav>
      </div>
    </header>
  );

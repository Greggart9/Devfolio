...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const tick=()=>{ rx+=(mx-rx)*.13; ry+=(my-ry)*.13; if(ring.current){ring.current.style.left=rx+"px";ring.current.style.top=ry+"px";} frame=requestAnimationFrame(tick); };
    if(dot.current){dot.current.style.left=mx+"px";dot.current.style.top=my+"px";}
    if(ring.current){ring.current.style.left=rx+"px";ring.current.style.top=ry+"px";}
    window.addEventListener("mousemove",mv); frame=requestAnimationFrame(tick);
    ...
    return ()=>{ window.removeEventListener("mousemove",mv); clearInterval(t); cancelAnimationFrame(frame); };
  },[]);
